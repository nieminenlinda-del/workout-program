...
**Top work set:** among logged **work** sets of the matching lift, highest `weight_kg`; ties → highest `reps`; still tied → last such set. Assisted vertical pulls (`pull_up_cable`, `pull_up_band`) invert that: **lowest** assistance kg is the top set (hardest). Sets with `warmup: true` are excluded. Explicit `completed: false` is skipped; **missing** `completed` counts (legacy rows, same idea as missing session `status`). Display `Last: 50 kg × 5` (or `Last: BW × 6`, or `Last: 5 kg × 60s` for a weighted plank, or `Last: 10 kg assist × 6 · Cable` for assisted pull-ups). First sessions show muted `No prior log`.

If this PWA’s IndexedDB has no matching prior work for today’s T1 (empty Session log, or a saved session that is not Week 1 T1s — e.g. Safari vs home-screen store), every lift can stay **No prior log** even when Session log (1). That is missing Last:, not a week-boundary filter. Deleting the iOS home-screen icon wipes standalone IndexedDB; there is **no server-side recovery**. Home still shows **Restore Week 1 weights**, **Restore today’s Mon 14 Day A**, and **Import session JSON** on an empty install (and Restore Week 1 when Last: is missing); Session log always has the full backup card. Add to Home Screen again uses a new empty store; export JSON before deleting the icon. **Session JSON** export/import (Ravinto-style `linda-lift-sessions` backup) and **Restore Week 1 weights** (known T1s: squat 47.5×3×5, bench 40×3×5, DL 70×3×5, Fri bench 40×2×5; accessories are template placeholders) write rows into *this* store so Last: can resolve. **Restore today’s Mon 14 Day A** writes a reconstructed 2026-09-14 Day A (squat 57.5×3×4 @7 soft GREEN, RDL 50×8×3 @6, reverse lunge 37.5×8×3 @7, plank +5 kg 120s/120s/60s @7) labeled reference / reconstructed. Does not change Week 2 squat 57.5. Home-screen store ≠ Safari.
...
Cues: `src/domain/timerCue.ts` — `navigator.vibrate` first, then a Web Audio beep (may be silent if the phone is muted). Voice-on adds mixable Web Audio phrases at **30s**, **10s**, and **0s** (rest: done contour; interval: work / rest / done). iOS `speechSynthesis` is **not** used: it takes an exclusive spoken-audio session and stops Apple Music / Spotify. Before any beep, `navigator.audioSession.type` is set to `transient` (WebKit mixable Ambient) so gym music keeps playing or only briefly ducks. Each threshold fires once per countdown (lock-screen jumps cue only the lowest crossed mark). Skip does not cue 0s. Voice on/off lives on the rest card and interval setup (`localStorage` `linda-lift-timer-voice`, default on). Screen Wake Lock is requested while a timer is running (`useWakeLock`).

**iPhone Safari / PWA:** Completing a set (or Start on intervals) is the user gesture that unlocks Web Audio and claims the mixable session. Do **not** call `speechSynthesis.speak()` (including a silent warmup) — that stops background music immediately. While a countdown is running, `startTimerAudioKeepAlive` resumes a suspended `AudioContext` every 8s; it must not play silence (that would duck music on the interval). Residual: hardware mute still silences Web Audio (vibration remains); `audioSession` needs Safari 16.4+. TTS cannot mix with music on iOS.

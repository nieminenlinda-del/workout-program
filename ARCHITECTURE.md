...
**Top work set:** among logged **work** sets of the matching lift, highest `weight_kg`; ties → highest `reps`; still tied → last such set. Sets with `warmup: true` are excluded. Explicit `completed: false` is skipped; **missing** `completed` counts (legacy rows, same idea as missing session `status`). Display `Last: 50 kg × 5` (or `Last: BW × 6`, or `Last: 5 kg × 60s` for a weighted plank). First sessions show muted `No prior log`.

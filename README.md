...
1. **Today** — default **Monday / Day A** squat template. Pick A–D (Mon/Tue/Thu/Fri). Block A T1s follow the Kraft week table (Week 2 squat **57.5 × 3×4**; bench/DL hold load). The collapsed preview shows **work kg** (`57.5 kg · 3 × 4`) plus last week’s top work set (`Last: 47.5 kg × 5`) from completed logs — Start is not required. If Last: is missing on this install (empty Session log, or a saved session that is not Week 1 T1s), home still offers **Restore Week 1 weights** or **Import session JSON** without opening Session log. Safari ≠ home-screen IndexedDB.
...
7. **Save** — notes, persist `SessionLog` in IndexedDB behind a repository interface. **Session log** lists saved days, exports/imports JSON, and can restore Week 1 weights so Last: works on this install.

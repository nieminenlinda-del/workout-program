...
4. **Rest timer** — starts after a completed set using that set’s `rest_sec` (T1 longer, accessory/core shorter). Pause/resume, +15s / +30s, or skip to log the next set. Recovers from lock/background via wall-clock; buzzes + beeps when time is up (vibration does not need audio). Speaks 30s / 10s / done via Web Speech (Finnish voice if the phone has one). Voice on/off on the rest card.

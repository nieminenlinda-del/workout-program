...
1. **Today** — default template follows the weekday (Mon A / Tue B / Thu C / Fri D). Kraft trip week: Wed 23 Sep → C, Thu 24 → D, Fri 25 rest (no default D); Sun 20 is picker-driven (A then B). Block A T1s follow the Kraft week table (Week 3 from **20 Sep**: squat **60 × 3×3** @ ≤8, bench **42.5 × 3×4**, DL **72.5 × 3×3**, D **42.5 × 2×4**). The collapsed preview shows **work kg** plus last week’s top work set from completed logs — Start is not required. If Session log is empty (or Last: is missing), home shows **Restore Week 1 weights**, **Restore today’s Mon 14 Day A**, and **Import session JSON** above Start — do not bury import only inside Session log. Add to Home Screen again uses a new empty store; export JSON before deleting the icon. There is **no server-side recovery** (SessionLog is local IndexedDB only). Safari ≠ home-screen IndexedDB.
2. **Readiness** — sleep, soreness, energy, pain, motivation (1–10). Auto light `GREEN|YELLOW|RED` (override allowed). Pain flag.
3. **Workout** — seed lifts (T1 + Day D bench volume compute a Kraft warmup ladder from work weight W, labeled W1…), swap listed alternatives, override kg mid-session (working-weight stepper + set logger). Logging a set does **not** replace leftover planned kg; each row keeps its `Plan` until that set is logged. Opt-in can prefill leftover logger inputs with this kg. Last matching **work** set shows on the lift (`Last: 50 kg × 5` or `No prior log`). Log each set (`weight_kg`, `reps`, `rpe`, `completed`, optional `amrap` / `warmup`). Timed holds (plank / side plank) still log seconds, and **added kg** is allowed (`5 kg × 60s`; `0` stays BW). Day D pull-ups can switch to **cable-assisted** (home downward cable + plates) or band-assisted; that kg is **assistance** (`10 kg assist × 6`), not extra load. Unassisted stays BW / optional added weight on the bodyweight pull-up.
4. **Rest timer** — starts after a completed set using that set’s `rest_sec` (T1 longer, accessory/core shorter). Pause/resume, +15s / +30s, or skip to log the next set. Recovers from lock/background via wall-clock; buzzes + beeps when time is up (vibration does not need audio). Voice on plays mixable Web Audio cues at 30s / 10s / done so Apple Music / Spotify keep going (iOS Web Speech would stop them). Completing the set is the iPhone gesture that unlocks audio; leave **Voice on**. Keep the screen awake through the rest (don’t lock) for the first check. Voice on/off on the rest card.
5. **Interval timer** — same rounds × work / rest screen from **Today** and from an in-session **Interval timer** button on the workout screen (Back returns to Workout). Same buzz/beep plus mixable 30s / 10s / next-phase cues. Per-set rest overlay is unchanged.
...
7. **Save** — notes, persist `SessionLog` in IndexedDB behind a repository interface. **Session log** lists saved days, exports/imports JSON, can restore Week 1 weights so Last: works on this install, and can restore a reconstructed Mon 14 Day A (squat 57.5×3×4 @7, RDL 50×8×3 @6, reverse lunge 37.5×8×3 @7, plank +5 kg 120s/120s/60s @7). There is no server backup.
...
The last 14 days of **active energy** are shown with Linda’s training-day map: Mon=A, Tue=B, Thu=C, Fri=D, other days rest (Kraft trip week: Wed 23 Sep 2026 = C, Thu 24 = D, Fri 25 rest). Timezone for day buckets is **Europe/Helsinki**. Daily totals prefer `ActivitySummary.activeEnergyBurned` for that date; if none, they sum `ActiveEnergyBurned` samples.
...
Working weights are placeholders, not a TM calculator. Documented TMs: squat **72.5** / bench **52.5** / deadlift **90** kg.

| Day | Focus |
| --- | --- |
| Mon A | `squat_low_bar` T1, `rdl`, `reverse_lunge` (barbell+plates, seed 37.5 kg) or `goblet_squat`, `plank` or `dead_bug` |
...
| Fri D | `bench_regular_volume`, `front_squat_light` or `goblet_squat`, `pull_up` or `pull_up_cable` / `pull_up_band`, optional `curl_db` / `cable_rope_pushdown` (or band) |

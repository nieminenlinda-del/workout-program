...
1. **Today** — default **Monday / Day A** squat template. Pick A–D (Mon/Tue/Thu/Fri). Block A T1s follow the Kraft week table (Week 2 squat **57.5 × 3×4**; bench/DL hold load). The collapsed preview shows **work kg** (`57.5 kg · 3 × 4`) plus last week’s top work set (`Last: 47.5 kg × 5`) from completed logs — Start is not required. If Session log is empty (or Last: is missing), home shows **Restore Week 1 weights**, **Restore today’s Mon 14 Day A**, and **Import session JSON** above Start — do not bury import only inside Session log. Add to Home Screen again uses a new empty store; export JSON before deleting the icon. There is **no server-side recovery** (SessionLog is local IndexedDB only). Safari ≠ home-screen IndexedDB. Does not change Week 2 squat 57.5.

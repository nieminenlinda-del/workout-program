...
7. **Save** — notes, persist `SessionLog` in IndexedDB behind a repository interface. **Session log** lists saved days, exports/imports JSON, can restore Week 1 weights so Last: works on this install, and can restore a reconstructed Mon 14 Day A (squat 57.5×3×4, plank 5 kg; other lifts are Week 2 template placeholders). There is no server backup.
